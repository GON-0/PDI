...
#Definimos funcion para simplificar codigo de visualizacion de imagenes
...
#Notas:
    #Los umbrales los determinamos por heuristica, prueba y error
...
def obtener_celdas(img, ver_bin = False, ver_celdas = False):
    ...
    #Obtengo las coordenadas de las lineas verticales (4 en total)
    y1, y2, y3, y4 = i_cols[0], i_cols[1], i_cols[2], i_cols[3] 
    #Para evitar lineas de las celdas en el recorte de la imagen, defino una constante
    k = 4 

    n = 1
    for i in range(len(i_rows) - 1):

        #Obtenemos las coordenadas de las 2 lineas horizontales de la celda i + 1
        x1 = i_rows[i]
        x2 = i_rows[i+1]

        #Extraigo los crop de la imagen original a partir de las coordenadas previas
        img1 = img[x1+k:x2-k,y1+k:y2-k]
        img2 = img[x1+k:x2-k,y3+k:y4-k]
...
def obtiene_letra(rta, ver_bin = False, ver_letra = False):
    ...
    if cant_letras == 0 or cant_letras > 1: #Si no tengo letras o si tengo mas de una letra
        return ''
...
def imprimir_correccion(respuestas, correctas):
    #Imprime el resultado de cada pregunta y determina si esta aprobado el examen
    rtas = []
...
resultados = [] 
for n in range(1,6):
    img = cv2.imread('examen_' + str(n) +'.png', cv2.IMREAD_GRAYSCALE)
