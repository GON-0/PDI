#VISTO ! Falta detectar la ultima moneda
#que parametro puedo modificar para que la capte ?
#deberia aplicar alguna otra herramienta de pre-procesamiento ?
#umbralado para que resalten mas ?
...
imshow(gray)
#cv2.imshow('contornos', gray) #Estaba asi, lo cambie porque no me funciona
...
circles = cv2.HoughCircles(gray,cv2.HOUGH_GRADIENT_ALT, 1, 20, param1=50, param2=0.7, minRadius=0, maxRadius=50)  # Circulos chicos
circles = np.uint16(np.around(circles))
cf = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR) #No se porque pero transforma a RGB
for i in circles[0,:]:
    cv2.circle(cf, (i[0],i[1]), i[2], (255,0,0), 2)   # draw the outer circle
    cv2.circle(cf, (i[0],i[1]), 2, (0,0,255), 2)      # draw the center of the circle
imshow(cf)